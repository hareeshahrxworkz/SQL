...
insert into commonwealthgames values (1,'Crecket',11,'india','5','MS Dhini',20,10,'Relience','Banglore',10,50,20,'RCB','RR','CSK','CSK','Russia','RCB','CSK','Crecket');
insert into commonwealthgames values (2,'Crecket',12,'China','7','virat koihli',30,15,'Oppo','Chani',12,14,50,'Csk','RCB','RR','RR','China','CSK','RCB','Crecket');
insert into commonwealthgames values (3,'  Crecket',13,'  Srilanka','10','KR Rahul',10,25,'HP','Rajastan',25,21,11,'Astrielia','SRH','RCB','SRH','Bangladesh','Srh','SRH','Crecket ');
insert into commonwealthgames values (4,'   Crecket',  14,' Pakistan','15','   Vinayak',  28,  31,'   BH','   Maharastra',  12,26,17,'  MI','   BCR', '  MI','KRS   ','   Gokak','   BCR','KSC   ','Crecket   ');
insert into commonwealthgames values (5,'Cre1cket  ',15,'Chikkamagaluru','11','Deelep',25,12,'Vivo','Manglore',12,23,21,'R C B','R R','C SK','RCB','Aishia','R C B','C SK',' Crecket');
insert into commonwealthgames values (6,'Bascate Ball',16,'Bombe','9','MS ',21,21,'Bharat','Goa',21,25,32,'JcB','GOv','CSK','CSK','OINDIA','RCB','CSK','Bascate Ball');
insert into commonwealthgames values (7,' Bascate Ball',17,'mumbai','8','MS virat',22,22,'Shell','rajastan',32,12,26,'GJ','RRE','TRE','TGR','Odisa','LCR','GHR','Basca te Ball');
insert into commonwealthgames values (8,'  Bascate Ball',18,'gujarah','6','MS Rajini',24,9,'vidol','mumbai',11,15,14,'MMM','MNC','FRT','KSC','Srilnaka','LCM','SBC',' Bascate Ball');
insert into commonwealthgames values (9,'    Bascate Ball',19,'Rajastan','18','manjunatj',7,6,'Escort','hydarabad',21,32,18,'AAA','AGR','JUR','HTU','Boothan','LMR','Hse','  Bascate Ball');
insert into commonwealthgames values (10,'Bascate Ball  ',21,'Assam','17','jadeja',2,1,'Realme','Srilankaore',26,24,27,'VTU','HYU','HYR','KPS','Maldives','LTD','GPH','Bascate Ball ');
insert into commonwealthgames values (11,'kabadi',22,'Goa','12','ramu',20,10,'Redmi','Shivamogga',21,28,35,'ADF','KES','UTV','MUS','Pashimabangala','GJJ','BCD','kabadi');
insert into commonwealthgames values (12,' kabadi',23,'Chanai','16','KlRuhalhini',1,3,'Samsung','Rajajinagar',35,65,28,'RCB','MNJ','JKS','sss','Gujarat','NCN','GGK','kabadi ');
insert into commonwealthgames values (13,'kabadi ',24,'Astrelia','19','kulakarni',17,29,'Apple','Gurugan',40,18,12,'NGH','HTh','HGT','URKS','Buldif','GTH','GGR','kabadi  ');
insert into commonwealthgames values (14,'ka badi ',25,'Gombegal','13','deshpande',19,27,'Goa','newdelhi',11,36,38,'IRE','GGH','MMT','TRH','Bhoothon','HTY','RRC','   kabadi');
insert into commonwealthgames values (15,' kabadi ',26,'tatkal','21','Manikumar',27,32,'pepsi','ahamadabad',33,38,13,'BRC','RN','LLT','JUR','Gurugan','HYR','RRR','    kabadi');


select count(id),no_of_playares,game_name,bonous_point from commonwealthgames group by game_name;
select sum(id),no_of_playares,game_name,bonous_point from commonwealthgames group by game_name having bonous_point=6 ;
 ...
 select Caption_name from commonwealthgames order by Caption_name  ;
  select playar_ranking from commonwealthgames order by playar_ranking  ;
    select semi_final_team from commonwealthgames order by semi_final_team  ;
  select gametype from commonwealthgames order by gametype  ;
  select panalytes_points from commonwealthgames order by panalytes_points  ;

select reverse(Caption_name) from commonwealthgames;
select reverse(qulifier_team_winner) from commonwealthgames;
select reverse(sponcer) from commonwealthgames;
...
select * from commonwealthgames where id between 2 and 7;
select * from commonwealthgames where id between 9 and 15;
select * from commonwealthgames where id between 6 and 11;


select * from commonwealthgames where id not in(1,6);
select * from commonwealthgames where id not in(2,4);
select * from commonwealthgames where id not in(1,2);
select * from commonwealthgames where id not in(7,9);
select * from commonwealthgames where id not in(10,11);
select * from commonwealthgames where id not in(8,15);


select sum(no_of_medals) from commonwealthgames;
select avg(no_of_medals) from commonwealthgames;
select min(no_of_medals) from commonwealthgames;
select max(no_of_medals) from commonwealthgames;
select count(no_of_medals) from commonwealthgames;


SELECT game_name FROM commonwealthgames WHERE game_name  LIKE 'b%';
SELECT venue FROM commonwealthgames WHERE venue  LIKE 's%';
SELECT sponcer FROM commonwealthgames WHERE sponcer  LIKE 'i%';
SELECT semi_final_team FROM commonwealthgames WHERE semi_final_team  LIKE 'r%';
